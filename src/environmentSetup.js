...
const validateConfigFile = () => {
  ...
  var configJsonPath = path.join(__dirname, '../../..', app.prosGlobal.configFile);

  if (fs.existsSync(configJsonPath)) {
    try {
      const file = fs.readFileSync(configJsonPath);
    ...
    configErrors.push(
      `Configuration file config.json not found at location ${configJsonPath}`,
    );
  }
